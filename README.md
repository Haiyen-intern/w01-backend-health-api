...
Hướng dẫn chạy Local Để khởi chạy server ở môi trường local, bạn thực hiện theo các bước sau:
...
Đối với Windows (Git Bash / CMD): .\venv\Scripts\activate

Đối với macOS / Linux: source venv/bin/activate

Bước 3: Khởi chạy Backend server bằng lệnh: python server.py

Thông tin API Endpoint Sau khi server khởi chạy thành công, endpoint cục bộ sẽ sẵn sàng tiếp nhận yêu cầu:
...
{ "status": "OK", "message": "Backend server is running smoothly" }

Quy trình kiểm thử (Testing với Postman) Toàn bộ các test case phục vụ việc kiểm thử đã được cấu hình sẵn trong dự án:
